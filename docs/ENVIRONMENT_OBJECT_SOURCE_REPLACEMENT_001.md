...
The west Object proxy keeps its exact receiving identity:

- position `[-3.4, 4.6, 0.55] m`;
- reserved size `[1.1, 1.1, 1.1] m`;
- authored dressing rotation `18 degrees` around source Z / world vertical;
...
A structural PASS requires the exact source mesh to remain inside the reserved west Object footprint/height after the authored 18-degree dressing rotation, remain grounded, keep the readable path open, preserve minimum spacing, leave unrelated scene state exact and add exactly one `object-source` mesh.
...
A PASS proves only exact receiving fit, preserved multi-asset scene state and target-host visibility for this one source-owned Object structural mesh in this exact Map slot. It does **not** prove final Object materials, lid/latch motion, service-module attachment, UC GLB equivalence, collision, gameplay, target-device runtime budgets, final environment dressing, final Art Direction, CANON, production readiness or Environment/Object mastery.
