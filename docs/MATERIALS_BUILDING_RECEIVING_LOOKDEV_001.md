...
- Building Materials candidate: `mike-axiom-mir/axm-building-design#3` at `b08f683f1c3f75c474fb347e1d1990c1c4426a33`.
...
## Receiving-driven source refinement

The first exact Map A/B used the earlier Building Materials head `484ced313ba0337ea27eebd01c5677e72e8456af`. Its standalone Building proof was structurally and renderer-valid, but direct receiving-scene inspection exposed a hierarchy problem that the isolated proof could not classify: the original ochre service panels (`#9B7338`, metallic `0.28`, roughness `0.50`) became the strongest path-end color accent and increased contrast around the already-known triangular front-panel lighting/shadow read.

That was not corrected with a Map-only override. The source-owned Building Materials lane was refined instead, changing only `utility_panel_ochre` to `#6F5A3A`, metallic `0.18`, roughness `0.62`. Frame, infill, roof and slab remain unchanged. The revised Building source candidate independently reruns its own structural and three-context Godot proof before this Map lane consumes it.
