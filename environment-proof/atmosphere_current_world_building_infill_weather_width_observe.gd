...
const COMBINED_PAYLOAD_PATH := "res://generated/current_world_building_infill_weather_width.json"
const COMBINED_SCHEMA := "axm.environment-current-world-building-infill-weather-width-composition/v0.1"
const COMBINED_STATUS := "PASS_CURRENT_WORLD_BUILDING_INFILL_WEATHER_WIDTH_STRUCTURE"
...
const BUILDING_POLICY := "EXACT_SOURCE_OWNED_BUILDING_PLUS_INFILL_ALBEDO_SUCCESSOR_AND_SOURCE_WIDTH_WEATHER"
const BUILDING_MATERIAL_PROFILE := "0c4834bf0fc9c0b7aa1a053f35f307b64596e13f305287ca3018b6182b2f6fe9"
...
func add_building_material(root3d:Node3D,data:Dictionary)->Dictionary:
    ...
    if String(proof.get("receiving_policy",""))!=BUILDING_POLICY:
        fail("Building infill-successor receiving policy drift")
        return {}
...
func add_static_sources(root3d:Node3D,data:Dictionary,cull_target_asset_id:String)->Array:
    ...
    for source_value in data.get("additional_source_meshes",[]) as Array:
        ...
        if String(source.get("asset_id",""))==BUILDING_ASSET_ID:
            fail("neutral Building source must remain absent in infill-successor composition")
            return []
