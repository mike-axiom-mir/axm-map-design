...
const PAYLOAD_PATH := "res://generated/runtime_budget_scene.json"
const SETTLE_FRAMES := 16
const VALID_VARIANTS := ["proxy_baseline", "source_sapling_only", "source_sapling_weather"]

var receipt := {
    "schema":"axm.environment-runtime-budget-observation/v0.1",
    "proof_runtime":"Godot 4.7.2 GL Compatibility",
    "promotion_effect":"NONE",
    "truth_boundary":"One exact variant of the bounded Environment source slice is observed from the two already-authored fixed cameras. Runtime counters are proof-host comparative evidence only; they do not establish target-device FPS/frame-time, final budgets, final scene art, gameplay, physical weather, CANON, or mastery."
}

func output_path(variant:String)->String:
    return "res://runtime-budget-%s.json" % variant
...
func add_sapling(root3d:Node3D,data:Dictionary)->Dictionary:
    ...
    node.material_override=material
    root3d.add_child(node)
    return {"vertices":vertices.size(),"triangles":triangles.size()}
...
func capture(viewport:SubViewport,variant:String,context:String)->Dictionary:
    ...
    var path="res://runtime-budget-%s-%s.png" % [variant,context]
    if image.save_png(path)!=OK:
        return {"state":"FAIL_CAPTURE"}
    return {"state":"PASS","width":image.get_width(),"height":image.get_height(),"bytes":FileAccess.get_file_as_bytes(path).size()}

func _initialize()->void:
    var user_args:=OS.get_cmdline_user_args()
    var variant=String(user_args[0]) if user_args.size()>0 else ""
    if not VALID_VARIANTS.has(variant):
        fail(variant if variant!="" else "invalid","missing or unsupported runtime budget variant")
        return
    var payload:=load_payload()
    if payload.is_empty() or String(payload.get("schema",""))!="axm.environment-runtime-budget/v0.1":
        fail(variant,"missing or invalid runtime budget payload")
        return
    var data=payload["scene"] as Dictionary
    ...
    var sapling_stats={"vertices":0,"triangles":0}
    var weather_stats={"streaks":0,"surfaces":0}
    if variant=="proxy_baseline":
        add_proxy(root3d,payload["proxy_baseline_item"] as Dictionary)
    else:
        sapling_stats=add_sapling(root3d,data)
        if variant=="source_sapling_weather":
            weather_stats=add_weather(root3d,data)
    ...
    for context in ["path_eye","elevated_oblique"]:
        ...
        if shot.get("state")!="PASS":
            ...
            return
        contexts[context]={"state":"PASS","runtime":runtime_stats(),"capture":shot}

    receipt["state"]="PASS_SCOPED_RUNTIME_OBSERVATION"
    receipt["variant"]=variant
    receipt["variant_semantics"]=(payload["variants"] as Dictionary)[variant]
    receipt["receiving_head"]=String(payload.get("receiving_head",""))
    receipt["scene_digest"]=String(payload.get("scene_digest",""))
    receipt["payload_digest"]=String(payload.get("payload_digest",""))
    receipt["godot_version"]=Engine.get_version_info()
    receipt["contexts"]=contexts
    receipt["sapling"]=sapling_stats
    receipt["weather"]=weather_stats
    receipt["fixed_content"]={"other_scene_items":(data["items"] as Array).size(),"path_overlay":true,"same_lighting":true,"same_cameras":true}
    receipt["performance_acceptance"]="COMPARATIVE_PROOF_HOST_ONLY_NO_TARGET_BUDGET"
    write_receipt(variant)
    print("AXM ENVIRONMENT RUNTIME BUDGET ",JSON.stringify(receipt))
    quit(0)
