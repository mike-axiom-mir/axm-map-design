...
const RECEIPT := "res://environment-sapling-motion-runtime-receipt.json"
const STATES := ["neutral", "peak"]
const CONTEXTS := ["path_eye", "elevated_oblique"]

var receipt := {
    "schema":"axm.environment-sapling-motion-runtime/v0.1",
    "promotion_effect":"NONE",
    "truth_boundary":"Godot 4.7.2 GL Compatibility captures of exact neutral and peak receiving-scene payloads. Capture success proves only renderability of these bounded states; it does not prove physical wind, continuous playback, production deformation, final materials, performance budgets, gameplay, final world art, CANON, or mastery."
}
...
func load_scene_data(state:String)->Dictionary:
    var path="res://generated/%s_scene_runtime.json" % state
...
func add_sapling(root3d:Node3D,data:Dictionary)->Dictionary:
    ...
    return {
        ...
        "motion_state":String(sapling.get("motion_state","")),
        "sample_time_s":float(sapling.get("sample_time_s",-1.0)),
        "vfx_provenance":sapling.get("vfx_provenance",{})
    }
...
func capture_context(state:String,context:String,data:Dictionary)->Dictionary:
    ...
    var path="res://%s_%s.png" % [state,context]
...
func _initialize()->void:
    var states={}
    for state in STATES:
        var data:=load_scene_data(state)
        if data.is_empty():
            fail("missing or invalid exact scene payload for %s" % state)
            return
        var contexts={}
        for context in CONTEXTS:
            var row:=await capture_context(state,context,data)
            if row.get("state")!="PASS":
                fail("environment sapling motion capture failed for %s/%s" % [state,context])
                return
            contexts[context]=row
        states[state]={
            "scene_digest":String(data.get("scene_digest","")),
            "contexts":contexts,
            "source_integration":data["source_integration"],
            "weather_presentation":data["weather_presentation"]
        }
    receipt["state"]="PASS_TARGET_HOST_NEUTRAL_PEAK_CAPTURE_READY"
    receipt["godot_version"]=Engine.get_version_info()
    receipt["states"]=states
    receipt["receiving_head"]=String(load_scene_data("peak").get("receiving_head",""))
    write_receipt()
    print("AXM ENVIRONMENT SAPLING MOTION ",JSON.stringify(receipt))
    quit(0)
